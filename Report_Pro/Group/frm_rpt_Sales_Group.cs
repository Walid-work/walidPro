    ...
    public partial class frm_rpt_Sales_Group : Form
    {
        ...
        private void btn_Report_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dt_ = dal.getDataTabl_1(@"USE GroupDB SELECT  H.Branch_code,H.branch_name As name_,C.Currency_Code
		        ,sum((D.QTY_TAKE-D.QTY_ADD)*A.Weight) as Weight_
		        ,sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price)-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc)/100) as value_Foreign
		        ,sum(D.TAX_OUT-D.TAX_IN) as tax_Foreign
				,ROUND(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price)-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc)/100),0)/ISNULL(NULLIF(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*A.Weight),0 ),0),1),3)*1000 AS Average_Foreign

		        ,sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*isnull(R.Average_Rate,1))-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc*isnull(R.Average_Rate,1))/100) as value_
		        ,sum((D.TAX_OUT-D.TAX_IN)*isnull(R.Average_Rate,1)) as tax_
				,ROUND(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*isnull(R.Average_Rate,1))-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*isnull(R.Average_Rate,1)*D.total_disc)/100),0)/ISNULL(NULLIF(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*A.Weight),0 ),0),1),3)*1000 AS Average_
                ...
                "%' and isnull(P.Main,0) Like '"+ SC +
                "%' and  H.Branch_code like '" + UC_Branch.ID.Text +
                "%' and A.group_code  between (CASE WHEN '" + Uc_Group.ID.Text.Length + "' >3  then  '" +Uc_Group.ID.Text + "' else '" + Uc_Group.ID.Text + "'+'0' end) " +
                ...
                " group by H.Branch_code,H.branch_name,C.Currency_Code");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            ,ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*isnull(R.Average_Rate,1))-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc*isnull(R.Average_Rate,1))/100),0) as value_
            ,ROUND(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*isnull(R.Average_Rate,1))-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc*isnull(R.Average_Rate,1))/100),0)/ISNULL(NULLIF(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*A.Weight),0 ),0),1),3)*1000 AS Average_
           
            ,ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price)-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc)/100),0) as value_Foreign
	        ,ROUND(ROUND(sum((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price)-sum(((D.QTY_TAKE-D.QTY_ADD)*D.Local_Price*D.total_disc)/100),0)/ISNULL(NULLIF(ROUND(sum((D.[QTY_TAKE]-D.[QTY_ADD])*A.Weight),0 ),0),1),3)*1000 AS Average_Foreign
...
DataSet ds = new DataSet();
	...
	,Abs(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0) )AS Cr_Weight_sales
	,Abs(Round(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)) AS Cr_Value_sales
	,Abs(ROUND(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)/ISNULL(NULLIF(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0 ),0),1),3)*1000) AS Cr_Average_Sales
	,Abs(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.TAX_OUT-d.TAX_IN)* isnull(R.Average_Rate,1)) else 0 end),0)) as Cr_tax_sal
	,Abs(Round(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0) + ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN (D.TAX_OUT-d.TAX_IN) else 0 end),0)) as Tot_cr_sal
	
	,Abs(ROUND(SUM(CASE WHEN  D.Transaction_code  like 'XS%' and C.Payment_Type =  '11' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0)) AS Cash_Weight_sales
	,Abs(Round(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type  =  '11' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)) AS Cash_Value_sales
	,Abs(ROUND(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type  =  '11' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)/ISNULL(NULLIF(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0 ),0),1),3)*1000) AS Cash_Average_Sales
	,Abs(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type   =  '11' THEN ((D.TAX_OUT-d.TAX_IN)* isnull(R.Average_Rate,1)) else 0 end),0)) as Cash_tax_sal
	,Abs(Round(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type    =  '11' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0) + ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN (D.TAX_OUT-d.TAX_IN) else 0 end),0)) as Tot_cash_sal

	
	,Abs(ROUND(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code  like 'XS%' and C.Payment_Type='2' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0)) AS Cr_Weight_sales
	,Abs(Round(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)) AS Cr_Value_sales
	,Abs(ROUND(ROUND(SUM(CASE WHEN V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)/ISNULL(NULLIF(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0 ),0),1),3)*1000) AS Cr_Average_Sales
	,Abs(ROUND(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.TAX_OUT-d.TAX_IN)* isnull(R.Average_Rate,1)) else 0 end),0)) as Cr_tax_sal
	,Abs(Round(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0) + ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type='2' THEN (D.TAX_OUT-d.TAX_IN) else 0 end),0)) as Tot_cr_sal
	
	,Abs(ROUND(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code  like 'XS%' and C.Payment_Type Like  '1%' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0)) AS Cash_Weight_sales
	,Abs(Round(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)) AS Cash_Value_sales
	,Abs(ROUND(ROUND(SUM(CASE WHEN V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0)/ISNULL(NULLIF(ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN (D.QTY_TAKE-D.QTY_ADD) * A.Weight  ELSE 0 END),0 ),0),1),3)*1000) AS Cash_Average_Sales
	,Abs(ROUND(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN ((D.TAX_OUT-d.TAX_IN)* isnull(R.Average_Rate,1)) else 0 end),0)) as Cash_tax_sal
	,Abs(Round(SUM(CASE WHEN  V.xe_ ='3' and D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price* isnull(R.Average_Rate,1)) - ((D.QTY_TAKE-D.QTY_ADD) * D.Local_Price * D.total_disc* isnull(R.Average_Rate,1)) / 100  ELSE 0 END),0) + ROUND(SUM(CASE WHEN  D.Transaction_code like 'XS%' and C.Payment_Type Like  '1%' THEN (D.TAX_OUT-d.TAX_IN) else 0 end),0)) as Tot_cash_sal
  ...
  "%' GROUP BY FORMAT(C.g_date, 'MM-yyyy') ");
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ,sum((D.QTY_TAKE - D.QTY_ADD)*D.Local_Price*R.Average_Rate)-sum(((D.QTY_TAKE - D.QTY_ADD)*D.Local_Price*D.total_disc*R.Average_Rate)/100) as value_
                ,sum((D.TAX_OUT-d.TAX_IN)*R.Average_Rate) as tax_
                ,ROUND(ROUND(sum((D.QTY_TAKE - D.QTY_ADD)*D.Local_Price*R.Average_Rate)-sum(((D.QTY_TAKE - D.QTY_ADD)*D.Local_Price*D.total_disc*R.Average_Rate)/100),0)/ISNULL(NULLIF(ROUND(sum((D.QTY_TAKE - D.QTY_ADD)*A.Weight),0 ),0),1),3)*1000 AS Average_
		        ...
		        Inner Join  Currency_rate as R on R.Currency_Code = C.Currency_Code and  R.Currency_Year = C.Cyear 
            ...
            "%' and isnull(A.Category,'') in('" + R + "','" + F + "','" + C + "','" + P + "','" + S + "','" + Z + "','" + X + "') " +
            " and ISNULL(A.group_code,'')  between (CASE WHEN '" + Uc_Group.ID.Text.Length + "' >3  then  '" + Uc_Group.ID.Text + "' else '" + Uc_Group.ID.Text + "'+'0' end) and (CASE WHEN '" + Uc_Group1.ID.Text.Length + "' >3   then  '" + Uc_Group1.ID.Text + "' else  '" + Uc_Group1.ID.Text + "'+'z' end) " +
            " and isnull(KM_CODE_ACC,'') like case when '" + str_t + "'= 3 then '3%' else '%' end  and KM_CODE_ACC not like case when '" + str_t + "'= 2 then '3%' else '' end " +
        ...
        try {
            ...
            DataTable dt_ = dal.getDataTabl_1(@"USE GroupDB SELECT A.Ser_no,B.branch_name,A.G_date,p.PAYER_NAME
            ...
            ,sum(D.QTY_TAKE - D.QTY_ADD) As Qty_Add

            , ROUND(sum((D.QTY_TAKE - D.QTY_ADD) * D.Local_Price) - sum(((D.QTY_TAKE - D.QTY_ADD) * D.Local_Price * D.total_disc) / 100), 2) AS Value_Foreign
            , sum(D.TAX_OUT-D.TAX_IN) As Vat_Foreign
            
            , ROUND(sum((D.QTY_TAKE - D.QTY_ADD) * D.Local_Price*R.Average_Rate) - sum(((D.QTY_TAKE - D.QTY_ADD) * D.Local_Price * D.total_disc*R.Average_Rate) / 100), 2) AS Value
            , sum((D.TAX_OUT-D.TAX_IN)*R.Average_Rate) As Vat
        ...
        private void btnMonthelySales_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               groupPanel4.Visible = false;
                ...
                inner join wh_BRANCHES As H on H.Branch_code=d.Branch_code					  
                where cast(D.G_date as date) between '"+dTP1.Value.ToString("yyyy/MM/dd")+ "' and '" + dTP2.Value.ToString("yyyy/MM/dd") +
                "' and D.Item_no like '" + Items.ID.Text +"%' and ISNULL(A.group_code,'')  between (CASE WHEN '" + Uc_Group.ID.Text.Length + "' >3  then  '" + Uc_Group.ID.Text + "' else '" + Uc_Group.ID.Text + "'+'0' end) and (CASE WHEN '" + Uc_Group1.ID.Text.Length + "' >3   then  '" + Uc_Group1.ID.Text + "' else  '" + Uc_Group1.ID.Text + "'+'z' end) " +
                "And H.branch_code like '" + UC_Branch.ID.Text+"%' and c.acc_no like '%'  GROUP BY H.branch_code,H.branch_name order by H.branch_code"));
